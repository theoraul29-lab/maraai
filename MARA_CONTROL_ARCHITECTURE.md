...
- Testing Agent is configured as `admin_invoked`, allowed `project.typecheck`, `server.build`, `frontend.typecheck`, `frontend.build`, and `python.execute` (see `server/services/agent-runtime.ts`) — the last one is real arbitrary-script capability, not just typecheck/build, and it does not run autonomously (triggered explicitly, or auto-created as the validation gate after a `repository.apply_changes` task completes).
- Code Agent plans created from admin chat (`handleAutonomousCodeRequest`) or Mara's own Writers Hub/Missions module analyzers (`autoApplyModuleProposal`) are planned only — both stop at `waiting_approval`, identically to a plan created directly in Control Center. Neither self-approves; every plan requires a real admin clicking Approve on `POST /api/control/code-agent/plans/:id/approve` before anything is applied, committed, or pushed. See `server/services/autonomous-code-pipeline.ts`.
