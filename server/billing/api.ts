...
export function registerBillingApi(app: Express): void {
  // -------------------------------------------------------------------------
  // GET /api/billing/paypal/status  (admin-only diagnostic)
  // -------------------------------------------------------------------------
  app.get('/api/billing/paypal/status', async (_req: Request, res: Response) => {
    try {
      const status = await checkPayPalConnection();
      res.json(status);
    } catch (err) {
