  ...
  wss.on('connection', (ws: any, req: any) => {
    ...
    ws.on('message', (message: Buffer) => {
      ...
      switch (data.type) {
        ...
        // --- P2P browser compute handlers ---
        //
        // When the user's browser worker starts computing, we register their
        // node in the in-memory compute pool so the admin dashboard can show
        // active contributors. Actual task dispatch happens via HTTP poll
        // (GET /api/p2p/get-task → POST /api/p2p/submit-result).
    ...
    ws.on('close', () => {
      if (ws.userId) {
        ...
        // Drop the node from the browser compute pool on disconnect.
        unregisterComputePeer(ws.userId);
      } else {
