...
experiments, and learning cycles. If asked about experiments or strategy, be specific and data-driven.`;
  ...
  // `mara_growth_experiments` is defined once in server/db.ts (the single
  // source for the auxiliary/runtime tables, created at startup before any
  // route handler runs), so no CREATE TABLE is needed here. The dashboard
  // queries below tolerate a missing table anyway via sqlGet/sqlAll.
