...
export async function registerRoutes(
  ...
  app.get("/api/mara-feed", async (req: any, res) => {
    try {
      ...
      let userSavedIds: Set<number> = new Set();
      let userLikedIds: Set<number> = new Set();
      if (userId) {
        const saved = await storage.getSavedVideos(userId);
        userSavedIds = new Set(saved.map((s: any) => s.videoId));
        const likedRows = await db
          .select()
          .from(likesTable)
          .where(eq(likesTable.userId, userId));
        userLikedIds = new Set(likedRows.map((l: any) => l.videoId));
      }
      const feed = [];
  ...
  app.post("/api/videos/:id/save", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
  ...
  app.delete("/api/videos/:id/save", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Login required" });
      const videoId = Number(req.params.id);
  ...
  app.get("/api/videos/saved", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Login required" });
      const saved = await storage.getSavedVideos(userId);
  ...
  app.get("/api/creator/post-status", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Login required" });
  ...
  app.get("/api/creator/my-videos", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Login required" });
  ...
  app.post("/api/creator/post-reel", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Login required" });
  ...
  app.get("/api/creator/analytics", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Login required" });
  ...
  app.post("/api/moderate", (req: any, res) => {
    const { text } = req.body;
    if (!text)
  ...
  app.post("/api/mara-speak", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
  ...
  app.delete("/api/creator/videos/:id", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Login required" });
      const videoId = Number(req.params.id);
  ...
  app.get(api.chat.list.path, async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
  ...
  app.get("/api/profile/:id", async (req: any, res) => {
    try {
  ...
  app.get("/api/profile/:id/videos", async (req: any, res) => {
    try {
      const profileId = req.params.id;
      const allVideos = await storage.getVideos();
      const creatorVideos = allVideos.filter((v) => v.creatorId === profileId);
  ...
  app.post("/api/profile/:id/follow", async (req: any, res) => {
    try {
  ...
  app.post("/api/preferences/language", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) {
        return res.status(401).json({ message: "Login required" });
  ...
  app.get("/api/trading/access", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
      const tradingAccess = await storage.getUserTradingAccess(userId);
  ...
  function getTradingAmount(period: string): string {
    return period === "yearly" ? "100.00" : "10.00";
  }

  app.post("/api/premium/order", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
  ...
  app.post("/api/admin/orders/:id/confirm", async (req: any, res) => {
    if (!isAdmin(req)) return res.status(403).json({ message: "Forbidden" });
    try {
  ...
  app.post("/api/admin/orders/:id/reject", async (req: any, res) => {
    if (!isAdmin(req)) return res.status(403).json({ message: "Forbidden" });
    try {
      const orderId = Number(req.params.id);
      if (isNaN(orderId))
  ...
  app.get("/api/writers/my-pages", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
  ...
  app.get("/api/writers/page/:id", async (req: any, res) => {
    try {
  ...
  app.post("/api/writers/create", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
  ...
  app.patch("/api/writers/page/:id", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
      const id = parseInt(req.params.id);
  ...
  app.delete("/api/writers/page/:id", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
      const id = parseInt(req.params.id);
  ...
  app.post("/api/writers/ai-tip", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
  ...
  app.get("/api/admin/improvements", async (req: any, res) => {
    if (!isAdmin(req)) return res.status(403).json({ message: "Forbidden" });
    try {
  ...
  app.post("/api/admin/improve-mara", async (req: any, res) => {
    if (!isAdmin(req)) return res.status(403).json({ message: "Forbidden" });
    try {
  ...
  app.patch("/api/admin/improvements/:id/status", async (req: any, res) => {
    if (!isAdmin(req)) return res.status(403).json({ message: "Forbidden" });
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) return res.status(400).json({ message: "Invalid ID" });
  ...
  app.post("/api/notifications/read/:id", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
      const id = parseInt(req.params.id);
  ...
  // === COLLECTIONS ===
  app.get("/api/collections", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
  ...
  app.post("/api/collections", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
  ...
  app.delete("/api/collections/:id", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
      const id = parseInt(req.params.id);
  ...
  app.get("/api/collections/:id/videos", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
      const id = parseInt(req.params.id);
  ...
  app.post("/api/collections/:id/videos", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
      const id = parseInt(req.params.id);
  ...
  app.delete("/api/collections/:id/videos/:videoId", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
      const id = parseInt(req.params.id);
  ...
  // === MARA BRAIN ===
  app.post("/api/admin/mara-brain/run", async (req: any, res) => {
    if (!isAdmin(req)) return res.status(403).json({ message: "Forbidden" });
    try {
  ...
  app.get("/api/admin/mara-brain/logs", async (req: any, res) => {
    if (!isAdmin(req)) return res.status(403).json({ message: "Forbidden" });
    try {
  ...
  app.post("/api/admin/mara-brain/self-post", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
  ...
  app.get("/api/creator/analytics/detailed", async (req: any, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) return res.status(401).json({ message: "Unauthorized" });
