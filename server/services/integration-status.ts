...
export function readIntegrationStatus(): IntegrationStatus[] {
  ...
  const anthropicKeySource = process.env.ANTHROPIC_API_KEY ? 'env' : hasAnthropicApiKeyOverride() ? 'control-center' : null;
  const anthropicConfigured = anthropicKeySource !== null && process.env.ANTHROPIC_FALLBACK_ENABLED !== 'false';
  return [
    { id: 'ollama', state: ollamaConfigured ? 'CONFIGURED' : 'NOT_CONFIGURED', configured: ollamaConfigured, available: ollamaConfigured, reason: ollamaConfigured ? 'Primary provider; liveness is checked by the provider router.' : 'OLLAMA_BASE_URL/OLLAMA_MODEL not configured.' },
    { id: 'anthropic', state: anthropicConfigured ? 'CONFIGURED' : 'NOT_CONFIGURED', configured: anthropicConfigured, available: anthropicConfigured, reason: anthropicConfigured ? `Optional fallback, configured via ${anthropicKeySource === 'env' ? 'Railway env var' : 'Control Center'}.` : 'Optional — add a key in Control Center → Integrations to enable it, or leave unset to run on Ollama only.' },
