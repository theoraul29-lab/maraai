...
type SubPeriod = "monthly" | "yearly";
const PRICING: Record<
  SubPeriod,
  { amount: string; label: string; savings?: string }
> = {
...
function TradingChart({ data, labels }: { data: number[]; labels: string[] }) {
...
const EXPLANATIONS: Record<
  TopicKey,
  { title: string; text: string; icon: typeof TrendingUp }
> = {
  RSI: {
...
function TradingPaywall({ hasPending }: { hasPending?: boolean }) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [subPeriod, setSubPeriod] = useState<SubPeriod>("monthly");
  const [payMethod, setPayMethod] = useState<"paypal" | "bank">("paypal");
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex gap-3 mb-6">
        <button
          ...
          className={`flex-1 py-4 rounded-xl border-2 transition text-center ${
            subPeriod === "monthly"
        ...
        <button
          ...
          className={`flex-1 py-4 rounded-xl border-2 transition text-center relative ${
            subPeriod === "yearly"
              ? "border-cyan-500/50 bg-cyan-500/10"
              : "border-border/50 bg-card"
          }`}
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          {(Object.keys(EXPLANATIONS) as TopicKey[]).map((topic) => {
