...
import type { WriterPage } from "@shared/schema";
import {
  ArrowLeft,
  PenTool,
  BookOpen,
...
type Category = "story" | "poem" | "book" | "article" | "journal";
type View = "hub" | "create" | "my-pages" | "browse" | "read";
...
function WriterEditor({
  ...
  const [category, setCategory] = useState<Category>(
    (editPage?.category as Category) || "story",
  );
  const [coverImage, setCoverImage] = useState<string | null>(
    editPage?.coverImage || null,
  );
...
function PageReader({
  ...
  const { data: page, isLoading } = useQuery<WriterPage>({
...
export default function WritersHub() {
  ...
  const [editPage, setEditPage] = useState<WriterPage | null>(null);
  const [readPageId, setReadPageId] = useState<number | null>(null);

  const { data: myPages = [], isLoading: myLoading } = useQuery<WriterPage[]>({
  ...
  const { data: premiumData } = useQuery<{ isPremium: boolean }>({
