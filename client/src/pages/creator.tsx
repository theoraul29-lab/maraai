...
export default function CreatorDashboard() {
  ...
  const { data: postStatus } = useQuery<PostStatus>({
    queryKey: ["/api/creator/post-status"],
  });

  const { data: myVideos = [], isLoading: videosLoading } = useQuery<Video[]>({
  ...
  const isYoutube = (videoUrl: string) => videoUrl.startsWith("youtube:");
  const getYoutubeId = (videoUrl: string) => videoUrl.replace("youtube:", "");
