...
function ReelCard({ video, onFullscreen }) {
  const { user } = useAuth();
...
function FullscreenView({ video, onClose }) {
  const { user } = useAuth();
  const { toast } = useToast();
...
export function ReelsFeed({ videos: _fallbackVideos, onSwitchToGrid }) {
  ...
  const { user } = useAuth();
  const { t } = useLanguage();
  const maraAudioRef = useRef(null);
  ...
  const allVideos = data?.pages?.flatMap((p) => p.feed) || [];
