...
export function NotificationsPanel() {
  ...
  const markReadMutation = useMutation({
    mutationFn: (id) => apiRequest("POST", `/api/notifications/read/${id}`),
    onSuccess: () => {
          ...
          >
            {isLoading ? (
              ...
              notifications.map((notification) => (
                <div
                  key={notification.id}
                  className={`flex items-start gap-3 p-3 border-b last:border-b-0 cursor-pointer hover-elevate ${notification.read ? "opacity-60" : "bg-accent/30"}`}
                  onClick={() => {
