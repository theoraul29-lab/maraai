...
- Frontend Tailwind/PostCSS: `frontend/tailwind.config.js`, `frontend/postcss.config.js`
...
- Root `npm run dev` runs `tsx server/index.ts` (Vite middleware in dev).
- Root `npm run start` runs the **compiled** server: `node dist/server/index.js`.
- Root `npm run build` runs `build:frontend` (Vite → `dist/public`) then `build:server` (esbuild compiles `server/` + `shared/` → `dist/`).
- Feature handlers live in `server/modules/*` (imported by `server/routes.ts`). There is no `backend/` directory.
- `frontend/package.json` defines a standalone subproject workflow for the React app.

## Build for Production

```bash
npm run build   # frontend → dist/public, server → dist/server (esbuild)
npm start       # node dist/server/index.js (serves API + built frontend)
```
